...
pub(crate) mod coverage_support {
    ...
    use super::MagicValueType;

    /// Exercises all public conversion helpers.
    ///
    /// # Returns
    /// Names and width summaries for all variants.
    pub(crate) fn exercise_magic_value_types() -> Vec<String> {
        let variants = [
        ...
        let mut result: Vec<String> = variants
            .iter()
            .map(|value_type| {
                format!(
                    "{}:{:?}:{}",
                    value_type.name(),
                    value_type.numeric_width(),
                    value_type.uses_little_endian_order()
                )
            })
            .collect();
        result.push(format!("{:?}", MagicValueType::from_name("unknown")));
        result
    }
}
