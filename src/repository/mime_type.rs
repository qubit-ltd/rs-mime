...
pub(crate) mod coverage_support {
    ...
    use crate::{MimeGlob, MimeRepository, MimeType};

    /// Exercises metadata getters and super-type magic matching.
    ///
    /// # Returns
    /// Summary strings from MIME type helpers.
    pub(crate) fn exercise_mime_type_edges() -> Vec<String> {
...
<mime-info>
  <mime-type type="application/base">
    ...
    <magic priority="50">
      <match type="string" value="BASE" offset="0"/>
    </magic>
  </mime-type>
...
"#,
        ...
        vec![
            mime_type.description().unwrap_or("none").to_owned(),
            mime_type.aliases().len().to_string(),
            mime_type.globs().len().to_string(),
            mime_type.magics().len().to_string(),
            mime_type.super_types().len().to_string(),
            mime_type.preferred_extension().unwrap_or("none").to_owned(),
            mime_type.all_extensions().join(","),
            mime_type.matches_filename("Makefile").to_string(),
            mime_type.matches_filename("Cargo.toml").to_string(),
            child.matches_content(&repository, b"BASE data").to_string(),
            child.matches_content(&repository, b"MISS").to_string(),
            (mime_type == mime_type.clone()).to_string(),
        ]
    }
}
