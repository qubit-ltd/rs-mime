...
impl ServiceProvider<MediaStreamClassifierSpec> for TestMediaStreamClassifierProvider {
    fn create_configured(
        ...
        match self.behavior {
            ...
            TestProviderBehavior::Unavailable => Err(ProviderFailure::unavailable(
                MimeError::ClassifierUnavailable {
                    name: "test".to_owned(),
                    reason: "missing executable".to_owned(),
                },
            )),
            TestProviderBehavior::InitializationFailed => Err(
                ProviderFailure::initialization_failed(MimeError::ClassifierBackend {
                    backend: "test".to_owned(),
...
impl ProviderMetadata for TestMediaStreamClassifierProvider {
    fn descriptor(&self) -> ProviderDescriptor {
        ProviderDescriptor::new(
            ProviderId::new(self.id).expect("test provider ID should be canonical"),
        )
